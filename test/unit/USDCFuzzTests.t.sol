...
contract TestUSDC is Test {
    ...
    function testFuzz_minter_can_mint_and_balance_updates(
        uint256 amount
    ) public {
        ...
        assertEq(
            ...
            amount,
            "alice should have minted amount"
        );
        assertEq(
            token.totalSupply(),
            amount,
    ...
    function testFuzz_burn_reduces_balance_and_supply(
        uint256 mintAmt,
        uint256 burnAmt
    ) public {
        ...
        assertEq(
            token.balanceOf(alice),
            mintAmt - burnAmt,
            "Alice's balance should reduce correctly after burn"
        );

        assertEq(
            token.totalSupply(),
            mintAmt - burnAmt,
            "Total supply must decrease equally to burn amount"
        );
    }

    function testFuzz_burnFrom_respects_allowance_and_reduces_it(
        uint256 minted,
